...
Agrega as transações do usuário autenticado por mês e retorna estrutura
compatível com o tipo MonthlyData do frontend (src/types/financial.ts).
"""
import re
import logging
...
@router.get(
    ...
    description="Agrega transações do usuário autenticado por mês.",
)
def get_monthly_dashboard(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> dict:
    # Filtra APENAS transações do usuário atual
    transactions = (
    ...
    logger.info(
        "dashboard/monthly: %d meses para user=%s",
        len(result), current_user.email,
    )
    return result
